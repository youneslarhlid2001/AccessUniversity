...
export default function OrientationPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('orientation_progress')
    if (saved) {
      try {
        const parsed = JSON.parse(saved)
        setFormData(parsed.formData || formData)
        // setStep(parsed.step || 1) // On restart user at step 1 for better UX flow re-engagement, or keep it. Let's keep it.
  ...
  useEffect(() => {
    localStorage.setItem('orientation_progress', JSON.stringify({
      formData,
      step
    }))
  }, [formData, step])
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-20 pb-12 relative overflow-hidden">
      {/* Background Decor */}
      <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-electric-500/10 rounded-full blur-[100px] -translate-x-1/2 -translate-y-1/2 pointer-events-none"></div>
      <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-purple-500/10 rounded-full blur-[100px] translate-x-1/2 translate-y-1/2 pointer-events-none"></div>
      ...
      <div className="max-w-4xl mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-12 animate-fade-in-up">
          <h1 className="text-4xl lg:text-5xl font-bold text-slate-900 mb-4">
            Votre avenir, <span className="text-gradient">sur mesure</span>
          </h1>
          <p className="text-slate-500 text-lg max-w-2xl mx-auto">
            Laissez notre algorithme analyser votre profil pour identifier les meilleures opportunités académiques pour vous.
          </p>
        </div>
              ...
              {step === 2 && (
                ...
                >
                  <div className="text-center mb-8">
                    ...
                    <p className="text-slate-500">Où souhaitez-vous écrire le prochain chapitre de votre histoire ?</p>
                  </div>
              ...
              {step === 4 && (
                ...
                >
                  ...
                  <div className="pt-6 border-t border-slate-100 flex gap-4">
                    <Link href="/ecoles" className="btn-secondary flex-1 text-center">Voir tout le catalogue</Link>
                    <Link href="/register" className="btn-primary flex-1 text-center">Sauvegarder mon dossier</Link>
                  </div>
                </motion.div>
              )}
